...
- 網址說明
  - 格式：`http://主機.網域:端口`
  - 端口之分配詳見[對應關係表](https://github.com/sinotec2/AIEE/blob/master/NLP/AnythingLLM/add_users.md#對應關係表)。
...
- 一如其他chatbot的習慣，AnythingLLM的主畫面切分為2大部分，左側邊條除了設定(板手符號，只有管理者看得到)及說明外，就是工作區(workspace, LLM、經理級以上可以[新增](./AnyChat_mng.md#新增工作區))及其目錄下的主題(thread、所有人都可新增)
- 中間部分為對話區
...
- 密碼只有最低長度的限制，其餘並無要求。
- 目前沒有提供忘記密碼的自動化補救措施，管理者與經理都有權限可以修改任密碼，可洽其中任何一位。
...
- 經理可以選擇預設的語言模型，但**不能**新增或修改語言模型API金鑰。
