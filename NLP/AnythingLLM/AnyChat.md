...
- AnythingLLM被稱為是使用者所需最後一個終極聊天機器人，具有開放性、可規模化、雲端地端整合性、大型LLM與小型嵌入引擎的整合、會員制與廣播制聊天機器人的整合、開放性生成與封閉性資料庫查詢的整合、靜態檔案連結與動態[RAG](AnyChatDoc.md#rag)的整合等等特色。
...
- 內設(一般)使用者
  - 只能發送與管理員或[經理](AnyChat_mng.md)新增到的工作區的聊天。
...
- AnythingLLM目前沒有提供其他登入方式(如AD/LDAP等)的選項，只有其內部[管理者](AnyChat_adm.md)設定的帳密權限管理。
- 網址說明
  - 格式：`http://主機.網域:端口`
  - 主機：eng06
  - 網域：sinotech-eng.com
  - 端口為4位數，自3001~3014，各技術組有自己的端口，分配詳見[對應關係表](add_users.md#對應關係表)。
- 權限角色：都使用相同界面登入，並無差別。
- 帳號設計
  - 電郵名稱(@左側部分)、方便[批次產生](add_users.md)。
- 初設密碼：
  - 員編重複2次
...
- 一如其他chatbot的習慣，AnythingLLM的主畫面切分為2大部分，左側邊條除了設定(板手符號，只有[管理者](AnyChat_adm.md)看得到)及說明外，就是工作區(workspace, LLM、[經理](AnyChat_mng.md)級以上可以[新增](AnyChat_mng.md#新增工作區))及其目錄下的主題(thread、所有人都可新增)
- 中間部分為對話區
...
- 密碼只有最低長度的限制，其餘並無要求。
- 目前沒有提供忘記密碼的自動化補救措施，[管理者](AnyChat_adm.md)與[經理](AnyChat_mng.md)都有權限可以修改任密碼，可洽其中任何一位。
...
- 經理級以上人員可以看得到暫存區的檔案目錄及名稱(上載icon)，也可以選擇其他人上傳的檔案作為主題連結的對象。
...
- 如果語言模型出現“幻覺”回應、建議新增主題來更新記憶體。

### 檔案、網頁與資料的連結

1. 既有檔案的連結
  - 過去所有使用者開啟的檔案，點選後，會出現使用的按鍵，需要按下「儲存至工作區」、或向右箭頭後，將其移到右側、按下**儲存與嵌入**（save and Embed），才會被GPT用以強化文本的產生([RAG](AnyChatDoc.md#rag))
  - 如果大型檔案，建議加註工作區名稱，讓其他使用者知道可以切換到該工作區，直接使用既有[RAG](AnyChatDoc.md#rag)解讀結果，避免重複上傳、解讀。
